...
Website names are human-readable (e.g., google.com, webmd.com, belledbottoms.io, poppedcorn.co.uk, etc.) because they are meant to be understood, remembered, and typed by humans. However, underlying the human-readable website names, are actual machine addresses where the websites are ultimately stored, the opaque IP (internet protocol) addresses such as 165.43.12.9 or 127.0.0.1. IP addresses which are meant to be understood by machines. For example, if we wanted to go to google.com, we would ultimately end up at 8.8.8.8. But how does a machine take a website name and translate it to an IP address so that a browser can find it and you can see the right site?


Well, many times that translation has already been done for you because you’ve previously visited the site so it is stored in your browser cache. What happens if you’ve cleared the cache? The next recourse we have is in our OS where we store this translation as well. However, if that is gone we can sometimes rely on our Internet Service Providers (ISPs) to save that information for us. But what if you are trying to access this website from a cafe in a city on the opposite coast? The ISP would be different and you wouldn’t necessarily have that stored. Another complication is that website IP addresses can be static, meaning they don’t change, or they can be dynamic, meaning they can change from time to time. Another reason for all of these resources not having the answer is potentially because this is the first time you’ve visited the site. That’s all fine and dandy but who or what translated the website name to an IP address in the first place? Enter the DNS (Domain Name System).


What is the DNS? When you type a website url into a browser that you’ve never visited before your computer has to send out a request to the DNS so that it can get back an IP address. DNS is a decentralized hierarchy of servers that can give you information about the different sections of the url you are looking for. Eventually, you get back a definitive IP address for the site. Once your browser knows what IP address corresponds with the website you are trying to reach, it needs to establish a connection so that it can start receiving the packets of data that will be used to create a page on your browser. However, this is not the end of it. It is not a matter of sending data back and forth. There are certain rules or protocols that need to be observed.


TCP/IP: A lot of data that is requested over the internet can’t be sent all at once because it is too large, so it needs to be broken up into packets and reassembled by the receiving end. One of the major protocols, or rules, for sending and receiving data, used on the internet is TCP/IP which stands for Transmission Control Protocol. What makes TCP different from another protocol such as UDP is that it has an error check method that confirms that packets were received if not it retransmits. But what is sending the packets?


HTTP/HTTPS/SSL/TSL: When we click on a link or ask for a specific url our browser like linkedin.com, the client, is making a HTTP (Hypertext Transfer Protocol) request of a server at some other IP address. An HTTP request consists of several standardized parts. It tells the server what kind of request it’s making: is it just getting data, is it creating new data, is it updating data, or is it deleting data. The server responds with a status code that let’s it know the result: success with no additional information (200) and returns the requested data or success with a redirect (3xx), or could not find the page or resource (404), or server error (5xx). The problem with this kind of protocol (HTTP) is that if someone manages to intercept it, so-called “man in the middle” attacks, they can read any and all data that is being transmitted such as passwords, credit card numbers, and other personal information. A way to prevent this is to use another protocol that is encrypted end to end, such as HTTPS. However, for this to start working you need to set up the server with an SSL certificate which is like an online ID. Then an encryption algorithm is employed. Now the data is no longer in plain text.


Web Server, Application Server, Database, & Firewall: Before we get into more security measures we have to explain the concept of a server. In the most generic sense, a server provides services to clients who request it (e.g., email server, database server, etc.). A web server provides static HTML content to clients that request it. By contrast an application server handles more dynamic requests, such as ones that would require querying a database, which is a server that stores data. Sometimes you don’t want to allow access to your server that contains your site from certain countries or you want to block certain IPs from accessing it. Sometimes you want to prevent certain applications or processes from inside your server from access to the internet. All of this can be done with a firewall. A firewall is software which you can configure with rules to allow or forbid certain types of access whether inbound or outbound.


Load Balancer: Let’s say you have a website like Google which gets millions of visitors a day. You want to be able to handle that much traffic without any outages and little down time. You want to be able to provide a service that is reliable and available. However, having an architecture that consists of only one server you would not be able to handle that many requests and that many concurrent queries to the database. For this kind of situation you would need to have several exact copies of servers that host your website. However, you would still need to use the same IP address. This is when a load balancer is helpful. What is a load balancer? A load balancer is a piece of software on a server that knows how to re-route traffic to the appropriate server based on one of several algorithms. One example of a simple algorithms is roundrobin. This is when we send traffic to the next available server.
